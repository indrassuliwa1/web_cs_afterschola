...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-2xl border border-gray-100">
    ...
    <h1 class="text-3xl font-extrabold text-gray-800 mb-6 border-b pb-2">
        <i class="fas fa-edit mr-2 text-blue-600"></i> Edit Kelas: {{ $kelas->nama_kelas }}
    </h1>
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="harga" id="harga" value="{{ old('harga', $kelas->harga) }}" required min="0"
                           class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm p-3 focus:ring-blue-500 focus:border-blue-500">
                    @error('harga')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                {{-- Field Foto Kelas --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Foto Kelas Saat Ini</label>
                    @if ($kelas->foto_kelas)
                        <img src="{{ asset('uploads/kelas/' . $kelas->foto_kelas) }}" alt="Foto Kelas" class="h-24 w-24 object-cover rounded-lg mb-2 border">
                    @else
                        <p class="text-xs text-gray-500 mb-2">Tidak ada foto terpasang.</p>
                    @endif
                    
                    <label for="foto_kelas" class="block text-sm font-medium text-gray-700 mb-1">Ganti Foto Kelas (Opsional)</label>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="10" 
                          class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm p-3 focus:ring-blue-500 focus:border-blue-500">{{ old('deskripsi', $kelas->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-4 pt-4 border-t mt-6">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-pen-to-square mr-2"></i> Perbarui Kelas
                </button>
            </div>
        </div>
    </form>
