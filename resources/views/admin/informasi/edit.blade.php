...
<div class="max-w-4xl mx-auto animate-fade-in">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-3xl border-t-4 border-indigo-600">
        ...
        <h1 class="text-3xl font-extrabold text-gray-900 mb-6 flex items-center border-b-2 border-indigo-100 pb-3">
            <i class="fas fa-edit text-indigo-600 mr-3"></i> Edit Data **{{ ucfirst($activeCategory) }}**
        </h1>
            ...
            <div class="space-y-8">
                ...
                <div class="field-group">
                    ...
                    <textarea name="isi" id="isi" rows="10" 
                                class="block w-full border-2 border-gray-200 rounded-xl shadow-md p-3 transition duration-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 hover:border-indigo-400">{{ old('isi', $informasi->isi) }}</textarea>
                    @error('isi')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 border border-gray-100 rounded-xl bg-gray-50 shadow-inner">
                    ...
                    <div class="field-group">
                         <label class="block text-sm font-bold text-gray-700 mb-2 flex items-center">
                            <i class="fas fa-image mr-2 text-green-500"></i> Gambar Saat Ini
                        </label>
                        @if ($informasi->gambar)
                        ...
                        @else
                            <div class="h-10 flex items-center text-xs text-gray-500 mb-3 p-2 border rounded-md bg-white">
                                <i class="fas fa-minus-circle mr-2"></i> Tidak ada gambar terpasang.
                            </div>
                        @endif
                        ...
                        @error('gambar')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-xs text-gray-500 mt-1">Maks. 2MB (jpg, jpeg, png). Akan menggantikan gambar lama.</p>
                    </div>
                    ...
                    <div class="field-group">
                        ...
                        <input type="date" name="tanggal" id="tanggal" 
                                value="{{ old('tanggal', \Carbon\Carbon::parse($informasi->tanggal ?? $informasi->created_at)->format('Y-m-d')) }}"
                                class="block w-full border-2 border-gray-200 rounded-xl shadow-md p-3 transition duration-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 hover:border-indigo-400">
                        @error('tanggal')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
